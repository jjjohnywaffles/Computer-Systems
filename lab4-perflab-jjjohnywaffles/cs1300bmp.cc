...
int cs1300bmp_writefile(char *filename, struct cs1300bmp *image)
{
  ...
  for (int col = 0; col < width; col++) {
    for (int row = 0; row < height; row++) {
      ...
      rarray[offset] = image->color[COLOR_RED][row][col];
      garray[offset] = image->color[COLOR_GREEN][row][col];
      barray[offset] = image->color[COLOR_BLUE][row][col];
    }
  }

  int error = bmp_24_write(filename, image->width, image->height, rarray, garray, barray);
